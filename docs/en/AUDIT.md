...
**Release:** `is0.0.2` · **Developer:** AsliCode · **Branch:** `main` (synced) · **Audit date:** 2026-09-25
**Production-ready bar (as defined):** works, passes tests, successful production build, E2E verified, no third-party identity, on main, version is0.0.2 verified.
...
bun scripts/release-check.mjs   → is0.0.2 FINAL (identity + banned-term scan)
...
| 2 | **Versioning** | 🟡 | `is0.0.2` for all 12 components, `parse/compare/isCompatible/bump` implemented + tested; `VERSION_FORMAT_PATTERN` enforced. Gap: `/api/v1/version` serves `INITIAL_VERSIONS` const instead of the existing `getVersionSnapshot()` helper — silent drift risk once components bump independently (P3 fix). |
...
| 26 | **Main App** (Android) | ❌ | Only the identity component + `apps.buildEnqueue` queue exists; worker stubs builds as instant `success`. Plan: real build line (Capacitor/PWA wrapper producing signed APK) or honest removal from the is0.0.2 scope (needs owner decision). |
...
| Version is0.0.2 verified | ✅ RELEASE.json + /api/v1/version + release-check |
