#!/usr/bin/env bash
# GuardAsli is0.0.2 — fully automated installation wizard (no prompts)
...
echo ""
echo "═══════════════════════════════════════════"
echo "  GuardAsli is0.0.2 — Fully Automated Wizard"
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  SALT="$(rand_hex)"
  cat > "$ENV_FILE" <<EOF
# GuardAsli is0.0.2 — wizard $(date -u +%Y-%m-%dT%H:%MZ)
# Product: GuardAsli · Developer: AsliCode · Coded by AsliCode
