...
describe("هویت مرکزی", () => {
  ...
  test("نسخه اولیه همه اجزا is0.0.1 است", () => {
    for (const c of COMPONENTS) {
      expect(INITIAL_VERSIONS[c]).toBe("is0.0.1");
    }
  });
});
...
describe("نسخه‌گذاری isMAJOR.MINOR.PATCH", () => {
  test("پارس و اعتبارسنجی", () => {
    expect(isValidVersion("is0.0.1")).toBe(true);
  ...
  test("مقایسه و bump", () => {
    expect(compareVersions("is0.0.1", "is0.0.2")).toBe(-1);
    expect(compareVersions("is1.0.0", "is0.9.9")).toBe(1);
    expect(compareVersions("is1.2.3", "is1.2.3")).toBe(0);
    expect(bumpVersion("is0.0.1", "patch")).toBe("is0.0.2");
    expect(bumpVersion("is0.0.9", "patch")).toBe("is0.0.10");
